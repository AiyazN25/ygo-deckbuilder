...
class AllCardsControl extends React.PureComponent {
  ...
  handleMainCardClick = (card) => {
    this.handleCardClick(card, "main");
  };
  handleExtraCardClick = (card) => {
    this.handleCardClick(card, "extra");
  };
  ...
  showingSomeOfAllCardsTitle = (numCards) => {
    ...
    return (
      "Showing " + numCards + " of " + totalNumberOfCards + " total cards."
    );
  };
      ...
      cardActions = [
        {
          text: "To Main Deck",
          actionHandler: () =>
            this.props.sendToHandler(
              "deck",
              "main",
              this.state.currentViewedCard
            ),
        },
        {
          text: "To Main Trunk",
